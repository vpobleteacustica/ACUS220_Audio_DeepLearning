...
## Estado actual del proyecto (Clase 2)

Este repositorio corresponde a las clases de ACUS220 (Acústica Computacional con Python) y actualmente incluye:

- **Tiny dataset acústico** con 5 clases:
  - Batrachyla_leptopus  
  - Batrachyla_taeniata  
  - Calyptocephalella_gayi  
  - Pleurodema_thaul  
  - Porzana_spiloptera
- **Extracción de features clásicos**:
  - STFT en dB (`stft_db`)
  - Log-Mel spectrogram en dB (`mel_db`) ← **feature principal para la CNN**
  - MFCC (`mfcc`)
  - (Opcional) Banco gammatone (`gammatone_db`, si está disponible)
- **Partición reproducible del dataset** en `train / val / test`:
  - `scripts/210_split_tiny_dataset.py`
  - `scripts/220_build_tiny_dataset_from_split.py`
- **Entrenamiento de una CNN simple (TinyMelCNN)** usando:
  - Log-Mel dB como entrada
  - Validación cruzada por semillas:
    - `scripts/300_train_tiny_cnn_split.py` con `--seeds 42,7,123`
  - Métricas guardadas en:
    - `data/processed/tiny_specs_split/cnn_run_split/seed_<SEED>/`
- **Inferencia**:
  - Un solo WAV: `scripts/350_infer_one.py`
  - Carpeta completa (batch): `scripts/360_infer_batch.py`
- **Pipeline completo**:
  - Script bash: `scripts/2500_run_tiny_pipeline.sh`

> La **feature principal** que ve la CNN es el **Log-Mel spectrogram en dB (`mel_db`)**, que convierte el audio en una “imagen de energía en el tiempo–frecuencia”.  
> La red no sabe lo que son Hz o kHz: solo ve patrones de energía, bordes y formas que aprende a asociar con cada especie.